...
function MyApp() {
  ...
  const calculateDaysAndNights = (pickupDate, dropDate) => {
  ...
  const [daysAndNights, setDaysAndNights] = useState({ days: 0, nights: 0 });
  useEffect(() => {
    const { days, nights } = calculateDaysAndNights(pickupDate, dropDate);
    setDaysAndNights({ days, nights });
    // Other logic...
  }, [pickupDate, dropDate]);  
 // Inside the useEffect block for fetching data and calculating prices
...
useEffect(() => {
  const fetchData = async () => {
    ...
    modifiedData.forEach((item) => {
      if (distance) {
        ...
        const days = daysAndNights.days; // Fetch days from state
        const nights = daysAndNights.nights; // Fetch nights from state

        console.log('Distance:', distanceNum);
        console.log('Price per km:', pricePerKmNum);
        console.log('Days:', days);
        console.log('Nights:', nights);

        // Calculate the price based on distance and price per km
        const distancePrice = (distanceNum * pricePerKmNum*2).toFixed(2);
        ...
        const driverAllowance = (days * 500) + (nights * 500);
        console.log('Driver Allowance:', driverAllowance);

        // Compare the distance price and served days' price to determine the greater one
        let totalPrice = Math.max(distancePrice, servedDaysPrice).toFixed(2);

        // Add the driver allowance to the total price
        totalPrice = (parseFloat(totalPrice) + driverAllowance).toFixed(2);

        console.log('Final Price:', totalPrice);

        item.price = totalPrice; // Assign the calculated total price to the item
      }
    });
  ...
  fetchData();
}, [distance, daysAndNights]); // Include daysAndNights as a dependency


  useEffect(() => {
    // Filter data based on conditions including hour difference
      ...
      const filteredData = travelData.filter((item) => {
        ...
        if (typeMatch) {
          ...
          switch (selectedType.toLowerCase()) {
            case 'sedan':
              pricePerKm = 15; // Set price per kilometer for Sedan
              break;
            case 'minisuv':
              pricePerKm = 18; // Set price per kilometer for MiniSUV
              break;
            case 'suv':
              pricePerKm = 22; // Set price per kilometer for SUV
              break;
            default:
              break;
          }
  ...
  }, [distance, pickupDate, dropDate, selectedPassengers, selectedSuitcase, selectedType, travelData, duration]);
                    ...
                    <div className="hidden lg:block">
                        <div style={{ display: 'flex', flexDirection: 'row', }}>
                            <div style={{ width: '35%', margin: 'auto', fontFamily: 'Arial, sans-serif', backgroundColor: '#f2f2f2', padding: '20px', borderRadius: '5px', }}>
                                ...
                                <div>
                                    <p style={{ marginBottom: '10px', textAlign: 'center' }}><strong style={{ color: '#555', fontSize: '16px' }}>Service Type</strong> <br /> <span style={{ color: '#777', fontSize: '14px' }}>Mumbai National</span></p>
...
{dropDate}
  ...
  {selectedType.toLowerCase() === 'sedan' && (
   ...
   <span className="font-bold" >⦿</span>   Per KM: ₹17
      </p>
      <p className="text-gray-700 mb-2">
  ...
  {selectedType.toLowerCase() === 'minisuv' && (
    ...
    <p className="text-gray-700 mb-2">
    <span className="font-bold" >⦿</span>     Per KM: ₹20
      </p>
      <p className="text-gray-700 mb-2">
      <span className="font-bold" >⦿</span>     Driver Charges per night: ₹500 (In case the driver stays overnight)
      </p>
 ...
 <p className="text-gray-700 mb-2">
 <span className="font-bold" >⦿</span>       Per KM: ₹25
      </p>
      <p className="text-gray-700 mb-2">
      <span className="font-bold" >⦿</span>      Driver Charges per night: ₹500 (In case the driver stays overnight)
      </p>
  ...
  <div className="flex flex-col md:flex-row md:space-x-5 space-y-3 md:space-y-0">
    ...
    <div style={{ width: '100%', paddingLeft: '0px' }}>
      ...
      <button className="bg-[#541e50] hover:bg-emerald-600 w-full mt-2 mb-2 text-white font-bold py-2 px-4 rounded-xl" onClick={() => handleSelect(item)}>
          Book This Vehicle
        </button>
