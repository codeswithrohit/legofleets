...
function MyApp() {
  ...
  const { pickupLocation, dropoffLocation, pickupDate, distance, duration } = router.query;
  ...
  const handleFilter = () => {
    const filteredData = travelData.filter((item) => {
      ...
      // Adjust price based on selected vehicle type
      if (typeMatch) {
        // Update price based on pricePerKm
        item.price = parseInt(item.pricePerKm)
      }
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const data = await firebase.firestore().collection('CarData').where('services', '==', 'Mumbai To Pune Airport').get();
  ...
  const handleSelect = (selectedItem) => {
    ...
    router.push({
      ...
      query: {
        ...
        selectedService: 'Mumbai Airport To Pune  ',
      },
    });
  };
  return (
        ...
        <div className=" ">
          ...
          <div className="hidden lg:block">
            <div style={{ display: 'flex', flexDirection: 'row', }}>
              <div style={{ width: '35%', margin: 'auto', fontFamily: 'Arial, sans-serif', backgroundColor: '#f2f2f2', padding: '20px', borderRadius: '5px', }}>
                ...
                <div>
                  <p style={{ marginBottom: '10px', textAlign: 'center' }}><strong style={{ color: '#555', fontSize: '16px' }}>Service Type</strong> <br /> <span style={{ color: '#777', fontSize: '14px' }}>Mumbai Airport To Pune </span></p>
          ...
          <div className="md:hidden">
            ...
            <div className="bg-gray-200 p-2 rounded-md">
              <h2 style={{ textAlign: 'center', color: '#333', fontSize: '24px', marginBottom: '20px' }}>Summary</h2>
              <div>
                <p style={{ marginBottom: '10px', textAlign: 'center' }}><strong style={{ color: '#555', fontSize: '16px' }}>Service Type</strong> <br /> <span style={{ color: '#777', fontSize: '14px' }}>Mumbai To Pune Airport</span></p>
                {/* 
                                    <p style={{ marginBottom: '10px', textAlign: 'center' }}><strong style={{ color: '#555', fontSize: '16px' }}>Transfer Type</strong><br /> <span style={{ color: '#777', fontSize: '14px' }}>One Way</span></p> */}
                <p style={{ marginBottom: '10px', textAlign: 'center' }}><strong style={{ color: '#555', fontSize: '16px' }}>PICKUP LOCATION</strong><br /> <span style={{ color: '#777', fontSize: '14px' }}>{pickupLocation}</span></p>
                <p style={{ marginBottom: '10px', textAlign: 'center' }}><strong style={{ color: '#555', fontSize: '16px' }}>Drop-off location</strong> <br /><span style={{ color: '#777', fontSize: '14px' }}>{dropoffLocation}</span></p>
