...
function MyApp() {
    ...
    const [hours, setHours] = useState(0); // Initialize hours state with 0
    ...
    useEffect(() => {
      ...
      const timeDiff = Math.abs(dropDateTime - pickupDateTime);
    ...
    const handleFilter = () => {
      const filteredData = travelData.filter((item) => {
        ...
        let price = distanceNum * pricePerKmNum; // Calculate base price based on distance
    
        const pickupDateTime = new Date(pickupDate);
        const dropDateTime = new Date(dropDate);
        const timeDiff = Math.abs(dropDateTime - pickupDateTime);
        const calculatedHours = hours;
    
        // Consider the base price based on vehicle type
        switch (selectedType.toLowerCase()) {
          case 'sedan':
            price = 2600 + Math.max(calculatedHours - 8, 0) * 250; // Base price for 8 hours, then 250/hour extra
            break;
          case 'minisuv':
            price = 3400 + Math.max(calculatedHours - 8, 0) * 300; // Base price for 8 hours, then 300/hour extra
            break;
          case 'suv':
            price = 4300 + Math.max(calculatedHours - 8, 0) * 350; // Base price for 8 hours, then 350/hour extra
            break;
          default:
            break;
        }
    
        item.price = price.toFixed(2); // Update the price in the item
      ...
      const fetchData = async () => {
        ...
        modifiedData.forEach((item) => {
          if (distance) {
            ...
            if (distanceNum > 80) {
              // Change the price only if the distance is greater than 80
  ...
  console.log("Data",travelData)
    ...
    const handleSelect = (selectedItem) => {
      const { vehicleType, brand, price,  } = selectedItem;
...
<p style={{ marginBottom: '10px', textAlign: 'center' }}>
            ...
            filteredTravelData.map((item, index) => (
                <div key={index} className="relative mb-2 mt-2 max-w-xs md:max-w-3xl mx-auto">
                  ...
                  <div style={{ width: '70%', paddingLeft: '20px' }}>
                    ...
                    <button className="bg-[#541e50] w-full hover:bg-emerald-600 text-white font-bold py-2 px-4 rounded-lg mt-2" onClick={() => handleSelect(item)}>
                      Book This Vehicle
                    </button>
                  </div>
                  </div>
  ...
  {selectedType.toLowerCase() === 'sedan' && (
   ...
   <p className="text-gray-700 mb-2">
   <span className="font-bold" >⦿</span> After 8 hours, additional charges of ₹ 250 per hour will be added.
   </p>
   <p className="text-gray-700 mb-2">
   <span className="font-bold" >⦿</span> After 80 kilometers, extra charges of ₹ 15 per kilometer will be applied.
   </p>
 </div>
 </div>
  )}
  ...
  {selectedType.toLowerCase() === 'minisuv' && (
    ...
    <p className="text-gray-700 mb-2">
    <span className="font-bold" >⦿</span> After 80 kilometers, extra charges of ₹ 18 per kilometer will be applied.
    </p>
  </div>
  </div>
  )}
 ...
 <p className="text-gray-700 mb-2">
 <span className="font-bold" >⦿</span> After 8 hours, additional charges of ₹ 350 per hour will be added.
 </p>
 <p className="text-gray-700 mb-2">
 <span className="font-bold" >⦿</span> After 80 kilometers, extra charges of ₹ 22 per kilometer will be applied.
 </p>
</div>
</div>
  )}
