...
function MyApp() {
    ...
    const [selectedSuitcase, setSelectedSuitcase] = useState("2");
    ...
    const { pickupLocation, dropoffLocation, distance, duration, pickupDate } = router.query;
    ...
    useEffect(() => {
      const fetchData = async () => {
        ...
        const data = await firebase.firestore().collection('formData').get();
        const modifiedData = data.docs.map((doc) => doc.data());
        modifiedData.forEach((item) => {
          if (distance) {
            const distanceNum = parseFloat(distance.replace(/\D/g, ''));
            const pricePerKmNum = parseFloat(item.pricePerKm);
            item.price = (distanceNum * pricePerKmNum).toFixed(2);
          }
        });
    ...
    const handleSelect = (selectedItem) => {
        const { vehicleType, brand, price } = selectedItem; // Assuming selectedItem has type, brand, and price properties
    
        // Navigate to the contact page with selected vehicle type, brand, and price
        router.push({
          ...
          >
            ...
            />
                    ...
                    <div className="hidden lg:block">
                        <div style={{ display: 'flex', flexDirection: 'row', }}>
                            ...
                            <div style={{ width: '80%', backgroundColor: 'white', padding: '20px', borderRadius: '8px' ,marginLeft:'24px'}}>
                                <h2 className="text-emerald-500" style={{ textAlign: 'center', fontSize: '1.5rem', margin: '0 0 1rem' }}>Vehicle Filter</h2>
                    ...
                    >
                      ...
                      <option value="3">3</option>
                                    ...
                                    <div style={{ flex: '1', padding: '2rem', background: '#F3F4F6', marginLeft: '1rem', borderRadius: '8px',  }}>
                    ...
                    >
                      ...
                      <option value="3">3</option>
                                    ...
                                    <div style={{ flex: '1', padding: '2rem', marginLeft: '1rem', background: '#F3F4F6', borderRadius: '8px', }}>
                                        <label htmlFor="type" style={{ display: 'block', fontSize: '1rem', marginBottom: '0.5rem', color: '#4B5563' }}>Type</label>
                                        <select id="type" value={selectedType}
                      onChange={(e) => setSelectedType(e.target.value)} style={{ width: '100%', padding: '0.5rem', borderRadius: '4px', border: '1px solid green' }}>
           ...
           <div className="px-4 py-12 bg-white">
            ...
            filteredTravelData.map((item, index) => (
                ...
                <div className="flex flex-col md:flex-row md:space-x-5 space-y-3 md:space-y-0 rounded-xl shadow-lg p-3 border border-emerald-500 bg-white">
                  <div style={{ width: '30%' }}>
                    <img src={item.carImage} alt={item.brand} className="rounded-xl" />
                  </div>
                  <div style={{ width: '70%', paddingLeft: '20px' }}>
                    <h3 className="font-black text-gray-800 md:text-3xl text-xl">{item.brand}</h3>
                    <p className="text-4xl font-black font-normal text-emerald-500">{`₹${item.price}`}</p>
                    <div className="flex flex-row items-center" onClick={toggleMoreInfo}>
                      ...
                      {showMoreInfo ? (
                        <ChevronUpIcon className="h-10 w-10 text-emerald-500 ml-2 cursor-pointer" />
                      ) : (
                        <ChevronDownIcon className="h-10 w-10 text-emerald-500 ml-2 cursor-pointer" />
                      )}
                      <div className="flex flex-row ml-16">
                        <UserIcon className="h-5 w-5 text-gray-800 ml-2" />
                        <p className="text-gray-800 text-sm ml-1">{`${item.passenger} People`}</p>
                        <ShoppingBagIcon className="h-5 w-5 text-gray-800 ml-2" />
                        <p className="text-gray-800 text-sm ml-1">{`${item.suitcase} Bags`}</p>
                      </div>
                    </div>
                    {showMoreInfo && (
                      <p className="md:text-lg text-gray-500 text-base">{`The best kept secret of The Bahamas is the country's sheer size and diversity. With 16 major islands, The Bahamas is an unmatched destination`}</p>
                    )}
                    <button className="bg-emerald-500 w-full hover:bg-emerald-600 text-white font-bold py-2 px-4 rounded-xl mt-2" onClick={() => handleSelect(item)}>
                      Book This Vehicle
                    </button>
                  </div>
                </div>
                    ...
                    <div >
                                <h2 className="text-emerald-500 text-center mb-4 text-xl font-bold" >Vehicle Filter</h2>
           ...
           <div className="px-4 py-12 bg-white">
            ...
            filteredTravelData.map((item, index) => (
                <div key={index} className="rounded-xl shadow-lg border border-emerald-500 bg-white p-3 mb-4 mt-2">
  <div className="flex flex-col md:flex-row md:space-x-5 space-y-3 md:space-y-0">
    ...
    <div style={{ width: '100%', paddingLeft: '0px' }}>
      <h3 className="font-black text-gray-800 md:text-3xl text-xl text-center mt-2">{item.brand}</h3>
      <button className="bg-emerald-500 hover:bg-emerald-600 w-full mt-2 mb-2 text-white font-bold py-2 px-4 rounded-xl" onClick={() => handleSelect(item)}>
          Book This Vehicle
        </button>
      <p className="text-4xl font-black font-normal text-emerald-500 text-center">{`$${item.price}`}</p>
      ...
      <div className="flex flex-col items-center justify-center mt-2">
        ...
        <div className="flex flex-row items-center justify-center" onClick={toggleMoreInfo}>
          ...
          {showMoreInfo ? (
            <ChevronUpIcon className="h-6 w-6 text-emerald-500 ml-2 cursor-pointer" />
          ) : (
            <ChevronDownIcon className="h-6 w-6 text-emerald-500 ml-2 cursor-pointer" />
          )}
        </div>
        {showMoreInfo && (
          <div className="flex flex-col items-center justify-center">
            <p className="md:text-lg text-gray-500 text-base text-center mt-2">
              The best kept secret of The Bahamas is the countrys sheer size and diversity. With 16 major islands, The Bahamas is an unmatched destination
            </p>
          </div>
        )}
      </div>
